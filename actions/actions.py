...
class AskForNameAction(Action):
    ...
    def run(
        ...
        lang = tracker.get_slot("lang")
        if lang == "ar":
            dispatcher.utter_message(text="من فضلك اخبرني ماهو اسمك")
        else:
            dispatcher.utter_message(text="please tell me what's your name")
...
class AskForEmailAction(Action):
    ...
    def run(
        ...
        lang = tracker.get_slot("lang")
        if lang == "ar":
            dispatcher.utter_message(text="من فضلك اخبرني ماهو ايميلك")
        else:
            dispatcher.utter_message(text="please tell me what's your email...")
        ...
        if not slot_value:
            ...
            raise ActionExecutionRejection(self.name(),
                                           "form,failed to validateSlot {0} with action {1}".format(slot_value,
                                                                                                    self.name()))
        name, massage = name_test(slot_value)
        if name is None:
            dispatcher.utter_message(text=massage)
            return {"name": None}
        else:
    ...
    def validate_problem(
        ...
        if not slot_value:
            ...
            raise ActionExecutionRejection(self.name(),
                                           "form,failed to validateSlot {0} with action {1}".format(slot_value,
                                                                                                    self.name()))
        name, massage = problem_test(slot_value)
        if name is None:
            dispatcher.utter_message(text=massage)
            return {"problem": None}
        else:
    ...
    def validate_email(
        ...
        if not slot_value:
            ...
            raise ActionExecutionRejection(self.name(),
                                           "form,failed to validateSlot {0} with action {1}".format(slot_value,
                                                                                                    self.name()))
        name, massage = email_test(slot_value)
        if name is None:
            dispatcher.utter_message(text=massage)
            return {"email": None}
        else:
    ...
    def validate_phone(
        ...
        if not slot_value:
            ...
            raise ActionExecutionRejection(self.name(),
                                           "form,failed to validateSlot {0} with action {1}".format(slot_value,
                                                                                                    self.name()))
        name, massage = phone_test(slot_value)
        if name is None:
            dispatcher.utter_message(text=massage)
            return {"phone": None}
        else:
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if last_intent_name == "affirm_ask_exchange_slots":
            ...
            if lang == 'ar':
                ...
                massage = "your request has been saved"
                dispatcher.utter_message(text=massage)
        return [UserUtteranceReverted()]
